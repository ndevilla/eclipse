...
/*
	$Id: prokeys.c,v 1.10 2003/07/10 11:50:20 yjung Exp $
	$Author: yjung $
	$Date: 2003/07/10 11:50:20 $
	$Revision: 1.10 $
*/
 ...
 */
/*--------------------------------------------------------------------------*/
int conica_add_files_history(qfits_header * fh, framelist * filenames)
{
    char    value[73] ;
    int     i ;
        ...
        framelist       *   calibfiles)
{
    ...
    /* Initialize */
    ins = pfits_identify_insstr("naco");
    ...
    if (pro_type!=NULL) {
        qfits_header_add(fh, "HIERARCH ESO PRO TYPE", 
                pro_type, "product type", NULL) ;
    }
   ...
   /* Parameter Name:    PRO DATE */
    ...
    if (calibfiles != NULL) {
        for (i=0 ; i<calibfiles->n ; i++) {
            sprintf(cval, "HIERARCH ESO PRO REC%d CAL%d NAME", nb, i+1) ;
            ...
            if (calibfiles->type != NULL) {
                if (calibfiles->type[i] != NULL) {
                    sprintf(cval, "HIERARCH ESO PRO REC%d CAL%d CATG", nb, i+1);
